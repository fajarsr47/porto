...
const projects = ref([
    {
        ...
        desc: "Platform jual beli online dengan fitur keranjang belanja, pembayaran gateway, dan dashboard admin yang responsif.",
    ...
    },
    {
        ...
        desc: "Desain antarmuka dashboard admin untuk memantau analitik data penjualan secara real-time.",
    ...
    },
    {
        ...
        desc: "Halaman landing page modern dengan animasi scroll yang halus dan performa tinggi (SEO Friendly).",
        github: "https://github.com/fajar-sr47/startup-landing"
    },
    {
        id: 4,
...
<template>
    <div id="portofolio" class="relative mt-5 md:mt-20 p-6 md:p-10 w-full flex flex-col md:flex-row gap-2 border-white/10 bg-[#030E21]/60 rounded-3xl backdrop-blur-sm overflow-hidden md:overflow-visible">
        ...
        <div class="h-full w-full flex flex-col z-10">
            <div class="flex flex-row justify-between items-end mb-8">
                <div>
                    <h2 class="text-4xl md:text-5xl font-bold text-white mb-2">My Portofolio</h2>
                    <p class="text-gray-400">Beberapa project pilihan yang telah saya kerjakan</p>
                </div>
                ...
                <button 
                    ...
                    :class="isExpanded ? 'text-red-400 hover:text-red-300' : 'text-amber-300 hover:text-amber-400'"
                >
                    {{ isExpanded ? 'Show Less <-' : 'See More ->' }}
                </button>
            </div>
            
            <div class="w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 pr-0 md:pr-40 xl:pr-80">
                ...
                <div 
                    ...
                    :class="isExpanded ? 'border-red-400/30 hover:bg-red-900/10' : 'border-white/20 hover:bg-white/5'"
                    @click="toggleProjects"
                >
                    <span 
                        class="font-medium"
    ...
    <div v-if="selectedProject" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-[#091427] border border-white/10 w-full max-w-2xl rounded-2xl overflow-hidden shadow-2xl flex flex-col md:flex-row animate-fadeIn">
            <button @click="closeModal" class="absolute top-4 right-4 z-20 bg-black/50 hover:bg-red-500 text-white w-8 h-8 rounded-full flex items-center justify-center transition-colors">
                &times;
            </button>
            <div class="w-full md:w-1/2 h-48 md:h-auto relative">
                <img :src="selectedProject.image" class="w-full h-full object-cover" alt="Project Image">
            </div>
            <div class="w-full md:w-1/2 p-6 md:p-8 flex flex-col justify-center">
                <span class="text-amber-300 text-sm font-bold uppercase tracking-wide mb-2">{{ selectedProject.category }}</span>
                <h3 class="text-3xl font-bold text-white mb-4">{{ selectedProject.title }}</h3>
                <p class="text-gray-300 mb-8 leading-relaxed">
                    {{ selectedProject.desc }}
                </p>
                <a :href="selectedProject.github" target="_blank" class="w-fit flex items-center gap-2 bg-white text-black hover:bg-amber-300 px-5 py-2.5 rounded-lg font-bold transition-all transform hover:scale-105">
